...
def test_regime_filter_long_ok():
    ...
    ok, reason = regime_filter(ind1h, ind4h, "LONG", "trend_pullback")
    assert ok
    assert reason == ""
...
def test_regime_filter_short_fail():
    ind1h = {"price": 110, "ema20": 108, "ema50": 105, "ema200": 100, "atr": 1.0, "vol_ratio": 125}
    ind4h = {"price": 130, "ema20": 125, "ema50": 120, "ema200": 110, "atr": 1.2, "vol_ratio": 130}
    ok, reason = regime_filter(ind1h, ind4h, "SHORT", "trend_pullback")
    assert not ok
...
def test_no_middle_range_range_bounce_long():
    sig = {"entry": 90.5, "direction": "LONG", "strategy": "range_bounce"}
...
def test_effective_middle_avoid_pct_is_wider_for_trend_pullback():
    sig = {"strategy": "trend_pullback"}
    cfg = {"range_mid_avoid_pct": 0.30, "trend_pullback_mid_avoid_pct": 0.45}
    assert effective_middle_avoid_pct(sig, cfg) == 0.45


def test_score_signal_bounds():
    sig = {"direction": "LONG", "strategy": "trend_pullback", "entry": 100, "sl": 99, "tp": 103}
    ind = {
        "rsi": 49,
        "vol_ratio": 120,
        "macd_hist": 0.3,
...
def test_score_signal_penalizes_range_bounce_far_from_support():
    sig = {"direction": "LONG", "strategy": "range_bounce", "entry": 95, "sl": 94, "tp": 97.2}
    ind = {
        "rsi": 52,
        "vol_ratio": 82,
        "macd_hist": 0.1,
    ...
    score, reasons = score_signal(sig, ind, funding=0.0001, oi_change_pct=1.0, regime_ok=True)
    assert "range_not_near_support" in reasons
...
def test_signal_quality_filter_is_neutral_for_v2_lite_setups():
    sig = {"entry": 100, "direction": "LONG", "strategy": "trend_pullback"}
    ind = {"price": 100, "support": 98, "resistance": 104, "atr": 1.0, "vol_ratio": 100}
    ok, reason = signal_quality_filter(sig, ind, {}, oi_change_pct=0.5)
    assert ok
    assert reason == ""
...
def test_screen_coins_respects_blacklist_and_limit():
    class Exchange:
        def tickers(self):
            return [
                {"symbol": "BTCUSDT", "turnover24h": "100000000", "fundingRate": "0.0001", "price24hPcnt": "0.01"},
                {"symbol": "FARTCOINUSDT", "turnover24h": "90000000", "fundingRate": "0.0001", "price24hPcnt": "0.20"},
                {"symbol": "ETHUSDT", "turnover24h": "80000000", "fundingRate": "0.0001", "price24hPcnt": "0.015"},
            ]
...
def test_btc_context_filter_blocks_alt_countertrend():
    sig = {"direction": "SHORT", "strategy": "trend_pullback"}
    ...
    assert not ok
    assert reason == "btc_countertrend"
...
def test_regime_strategy_allowlist_respects_enabled_strategies():
    ind1h = {"price": 110, "ema20": 108, "ema50": 105, "ema200": 100, "atr": 1.0, "vol_ratio": 125}
    ind4h = {"price": 130, "ema20": 125, "ema50": 120, "ema200": 110, "atr": 1.2, "vol_ratio": 130}
    out = regime_strategy_allowlist(ind1h, ind4h, {"enabled_strategies": ["trend_pullback", "range_bounce"]})
    assert out == {"trend_pullback"}


def test_detect_signals_skips_disabled_strategies():
    ind = {
        "price": 102.0,
        "rsi": 51.0,
        "macd_hist": 0.02,
    ...
    sigs = detect_signals(
        ...
        ind1h=ind1h,
        ind4h=ind4h,
        cfg={"enabled_strategies": ["range_bounce"]},
    )
    assert sigs == []


def test_regime_strategy_allowlist_v2_lite_forces_only_v2_strategies():
    ind1h = {"price": 110, "ema20": 108, "ema50": 105, "ema200": 100, "atr": 1.0, "vol_ratio": 125}
    ind4h = {"price": 130, "ema20": 125, "ema50": 120, "ema200": 110, "atr": 1.2, "vol_ratio": 130}
    out = regime_strategy_allowlist(
        ind1h,
        ind4h,
        {"agent_profile": "v2-lite", "enabled_strategies": ["trend_pullback", "range_bounce"]},
    )
    assert out == {"trend_pullback"}
