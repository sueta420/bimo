# ИНСТРУКЦИЯ ДЛЯ OPENCLAW — BYBIT FUTURES TRADER
# Выполни все шаги по порядку. После каждого шага показывай мне вывод.

---

## ШАГ 1 — Удали старые файлы и создай папку заново
...
## ШАГ 2 — Установи все зависимости через python3.11

```bash
/opt/homebrew/opt/python@3.11/bin/python3.11 -m pip install pybit openai pandas numpy ta
```

---

## ШАГ 3 — Проверь что всё установлено

```bash
/opt/homebrew/opt/python@3.11/bin/python3.11 -c "import ta, pybit, openai, pandas, numpy; print('OK — все установлено')"
```

Если видишь `OK — все установлено` — переходи к шагу 4.
Если ошибка — покажи мне вывод.

---

## ШАГ 4 — Создай файл с API ключами

```bash
cat > ~/trading/.env << 'EOF'
export BYBIT_API_KEY="ВСТАВЬ_BYBIT_KEY"
export BYBIT_API_SECRET="ВСТАВЬ_BYBIT_SECRET"
export OPENAI_API_KEY="ВСТАВЬ_OPENAI_KEY"
EOF
echo "Файл .env создан"
```

Затем открой и замени заглушки на реальные ключи:
```bash
nano ~/trading/.env
```
...
## ШАГ 5 — Скачай файл агента

```bash
curl -L "https://raw.githubusercontent.com/antropics/agent/main/futures_agent.py" -o ~/trading/futures_agent.py 2>/dev/null || echo "curl не сработал"
```

Если curl не сработал — создай файл вручную командой из шага 5б ниже.

## ШАГ 5б — Создай файл агента вручную (если шаг 5 не сработал)

```bash
cat > ~/trading/futures_agent.py << 'PYEOF'
"""
...
PYEOF
echo "Файл создан. Строк: $(wc -l < ~/trading/futures_agent.py)"
```

---

## ШАГ 6 — Проверь синтаксис файла

```bash
/opt/homebrew/opt/python@3.11/bin/python3.11 -c "
import ast
with open('/Users/$(whoami)/trading/futures_agent.py') as f:
    src = f.read()
ast.parse(src)
print('Синтаксис OK, строк:', src.count(chr(10)))
"
```

Если видишь `Синтаксис OK` — переходи к шагу 7.

---

## ШАГ 7 — Запусти агента

```bash
source ~/trading/.env
cd ~/trading
/opt/homebrew/opt/python@3.11/bin/python3.11 futures_agent.py
```

Ты должен увидеть примерно:
```
🤖 Агент запущен [TESTNET] | Ctrl+C для остановки

2026-... INFO ====...
2026-... INFO FUTURES AGENT START | TESTNET
2026-... INFO Model:gpt-4o-mini | $5 R:R=1:3 | max:5/day
2026-... INFO ── Скрининг...
2026-... INFO Топ монеты: ['BTCUSDT', 'ETHUSDT', ...]
```
...
```bash
# Смотреть логи
tail -f ~/trading/agent_$(date +%Y-%m-%d).log

# Остановить агента (закроет позиции + выдаст отчёт)
Ctrl+C

# Посмотреть отчёт
cat ~/trading/report_$(date +%Y%m%d).txt
```

## ПЕРЕКЛЮЧЕНИЕ НА РЕАЛЬНЫЕ ДЕНЬГИ

```bash
# Найти строку "testnet": True и заменить на False
sed -i '' 's/"testnet": *True/"testnet": False/' ~/trading/futures_agent.py
```

⚠️ Только после минимум 1 недели тестнет с хорошими результатами!
